...
const StudyAssistant = () => {
  ...
  const [currentView, setCurrentView] = useState<"upload" | "analysis" | "questions" | "quiz" | "quick-analysis" | "pdf-page-select" | "comprehensive-pdf" | "pdf-navigator" | "pdf-resume-options">("upload");
    ...
    if (pdfFile) {
      try {
        ...
        if (totalPages > 0) {
          ...
          // Check for existing study history for this PDF
          const existingHistory = await getStudyHistoryForFile(user?.uid || '', pdfFile.name);
          
          if (existingHistory && existingHistory.pageAnalysesMap) {
            ...
            setCurrentView("pdf-resume-options");
          } else {
  ...
  const analyzePdfFile = async (file: File, startPage?: number, endPage?: number) => {
    ...
    try {
      ...
      if (startPage && endPage) {
        ...
        setAnalysisStep(`Analyzing pages ${startPage} to ${endPage}...`);
      } else {
        setAnalysisStep("Analyzing PDF content...");
      }
  ...
  const analyzeComprehensivePdf = async (file: File) => {
    ...
    try {
      ...
      setAnalysisStep("Processing all PDF pages...");
      
      const result = await analyzePdfContentComprehensive(fullText, outputLanguage);
      
      setAnalysisProgress(90);
      setAnalysisStep("Compiling comprehensive results...");
  ...
  const handlePdfPageRangeSelect = (startPage: number, endPage: number) => {
    if (pdfInfo) {
      // Clear existing history when starting new analysis
      setExistingStudyHistory(null);
      // Start analyzing the selected page range
          ...
          <div className="container mx-auto max-w-4xl">
            <div className="text-center mb-8 animate-fadeInUp">
              <div className="flex items-center justify-center gap-3 mb-6">
                <div className="p-4 bg-gradient-to-r from-primary to-primary-glow rounded-full pulse-glow shadow-2xl">
  ...
  if (isAnalyzing) {
    const analysisType = selectedFiles.some(f => f.type === 'application/pdf') 
      ? currentView === "comprehensive-pdf" ? "comprehensive" : "pdf"
      : "detailed";
    
    return (
      <ModernAnalyzingState
        files={selectedFiles}
